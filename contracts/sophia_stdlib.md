...
Returns `Some` of a list without it's first element or `None` if the list is empty.
...
Finds any (first) element of `l` fulfilling predicate `p` as `Some` or `None` if no such element exists. 
...
Gets `n`th element of `l` as `Some` or `None` if `l` is empty.


#### get
`get(n : int, l : list('a)) : 'a`

Gets `n`th element of `l` forcefully, throwing and error if `l` is empty.
...
Assuming that cmp represents `<` comparison, inserts `x` before the first element in the list `l` which is greater than the argument. For instance,
```
...
Checks if any element of a list fulfill predicate `p`.
...
"zips" two lists with a function. n-th element of resulting list will be equal to `f(x1, x2)` where `x1` and `x2` are n-th elements of `l1` and `l2` respectively. Will cut tail of the longer list. For instance
```
...
Sorts a list using given comparator. `lesser_cmp(x, y)` should return `true` iff `x < y`. Requires `lesser_cmp` to represent linear order. Currently O(n^2).


#### intersperse
`intersperse(delim : 'a, l : list('a)) : list('a)`

Intersperses `l` elements with `delim`. Does nothing on empty lists and singletons. for instance
```
...
This module is used only to support syntactic sugars for lists. All its features are included in the [List](#List) namespace. Therefore, it should not be included manually and used explicitly.
...
Applies arity 2 function on two `option`s and returns `Some` iff `o1` and `o2` were `Some` or `None` in the other case. For instance
```
...
Turns `o` into an empty or singleton list.


#### filter_options
`filter_options(l : list(option('a))) : list('a)`

Removes `None`s from list and unpacks all `Some`s left. For instance
```
...
Out of two `option`s choose the one that is `Some`, or `None` if neither of them is.
...
Applies functions over respective element.
