...
This document describes miner signalled consensus upgrade. It is a mechanism
that actives a new hard forking consensus on the æternity network when the
majority of mining power supports it.

## Motivation

There have been only *planned hard forks (unconditional)* that were well
communicated in advance and include only consensus breaking features that the
whole community agree with.

Since the æternity is a complex platform that consists of components such as
naming system, oracles, smart contracts, generalized accounts and state
channels, the development of these components and the blockchain itself is
likely to be consensus breaking in the future. Moreover, the Lima hard fork is
the last planned hard fork, so planned hard forks must be replaced with another
mechanism.

To prevent the blockchain from a split, we chose to use the consensus upgrade
based on the miner signalling outcome. The vast majority (90% or more) of mining
power must be supporting the new consensus in order to upgrade the
blockchain. The minority can still follow the old consensus, but since its
mining power will be very low, this fork is generally supposed to become
abandoned.

Miner signalled consensus upgrade is a *conditional hard fork* since the
activation of the new consensus is based on a condition. A hard-fork (in the
context of a consensus protocol upgrade based on miner signalling) happens with
a key block, therefore, using the term "block" in the following text refers to a
"key block".
...
the new consensus becomes active at a determined height if at least a determined
majority of the blocks between two determined block heights (*signalling
interval*) contains a predefined *signal*. Such consensus upgrade happens per
each chain fork.

The signal value is part of a block header in the signalling interval. It:
- is meant to be a unique number, so to limit the risk of the same value being
  used by simultaneous competing consensus proposals;
- is not necessarily part of the validation rules of the new consensus
  ...
  validation rules of the new consensus protocol. This means that if the network
  reasonably settles on activating the new consensus protocol and a user starts
  a node configuring the new consensus protocol as unconditional at the same
  height as the (past) miner signalled consensus protocol upgrade height, the
  node may - depending on the chain forks observed on the network - follow as
  best valid chain a chain fork that upgraded to the new consensus protocol at
  the configured height even if without a majority of blocks with expected
  signal in the signalling interval.
...
NOTE: HS is included in the signalling interval and HE is excluded from it.  For
example, if HS is 100 and HE is 200, there are 100 blocks between (including HS,
excluding HE) and so it is easier to define the majority, i.e. 90 blocks out of
100 (as opposed to 99 or 101). The fork height in this case is 200.
...
protocol to be activated depending on miner signalling outcome. At node startup,
the node must parse, validate and load such configuration.
...
A node that has configuration for miner signalled consensus upgrade must be
...
The block header field `info` is used for storing the signal in blocks whithin
the signalling interval.
...
settles on whether to activate the new consensus protocol, the user is meant to
enforce such decision on the eventual node restart either by configuration or by
upgrading the node to a new version.
...
interpreting such `info` field as a bit mask rather than a value.
